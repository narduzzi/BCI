function plotCombiMap(r, varargin)
...
% $Revision: 0.4 $ $Date: 03/11/2009 14:45:00 $
% E-Mail: clemens.brunner@tugraz.at

% Revision history:
%   0.4: Add 'range' option to plot only a specific segment of time.
%   0.3: Adapt to new version of toolbox
%   0.2: Initial version based on plotErdsMap.m
...
a1 = cell(1, length(plot_index));  % Contains the axes of the ERDS subplots
a2 = cell(1, length(plot_index));  % Contains the axes of the average subplots
a3 = cell(1, length(plot_index));  % Contains the axes of the variance subplots
...
for i_rows = 1:n_rows
    for i_cols = 1:n_cols
        if sum(counter_total == plot_index) == 1
            ...
            a2{counter_plots} = axes('position', [border + border_plots + i_width * (i_cols - 1), plot_area + border + border_plots - i_height * i_rows + 2 * (i_height - border_plots) ./ 3, i_width - border_plots, (i_height - border_plots) ./ 6]);
            ...
            set(f, 'CurrentAxes', a3{counter_plots});
            set(gca, 'FontSize', font_size/1*3*2);
            ...
            set(a1{counter_plots}, 'ydir', 'normal');
            set(a2{counter_plots}, 'XTickLabel', '');
            set(a3{counter_plots}, 'XTickLabel', '');
            ...
            if sum(counter_total - 1 == plot_index) == 1 && i_cols ~= 1  % Draw y-labels only if there is no subplot to the left
                ...
                set(a3{counter_plots}, 'YTickLabel', '');
            end;
...
for k = 1:length(a2)
    axis(a2{k}, [r.t_plot(1), r.t_plot(end), peaks1(1), peaks1(2)]);
    % Draw line for cue
    if isfield(r, 'cue')
...
for k = 1:length(a3)
    axis(a3{k}, [r.t_plot(1), r.t_plot(end), peaks2(1), peaks2(2)]);
    % Draw line for cue
    if isfield(r, 'cue')
        set(f, 'CurrentAxes', a3{k});
        v = axis;
        line([r.cue, r.cue], [v(3), v(4)], 'Color', 'k');
    end;
end;
...
temp_str{1} = ['{\bfERDS maps 0.7.}{\rm Calculated on ', r.date_calc, '.}'];
